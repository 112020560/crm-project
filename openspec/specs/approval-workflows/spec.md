...
Defines configurable, auditable multi-step approval chains for credit applications. A `WorkflowDefinition` contains an ordered list of `WorkflowStep` records. When a CreditApplication enters `InReview`, the active workflow is associated with it and agents advance the chain step-by-step via the approve/reject endpoints. A single-agent fallback applies when no workflow is active.

## Requirements

### Requirement: Define a workflow definition with ordered steps
The system SHALL allow defining a `WorkflowDefinition` with an ordered list of `WorkflowStep` records. Each step has a name and an optional required role. A workflow definition starts in `Draft` status. Only one `WorkflowDefinition` SHALL have `Active` status at any time; activating a new one automatically sets the previous active definition to `Superseded`.
