...
Defines the evaluation lifecycle — triggering an evaluation, applying the active risk matrix, producing a `ScoreCard` and `RiskEvaluation`, driving automatic application status transitions, and publishing events. Includes the `POST /api/v1/risk-evaluations` endpoint for on-demand re-evaluation.
