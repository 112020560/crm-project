...
Defines how documents are registered, retrieved, and validated across the system. Documents use a polymorphic ownership model (`OwnerId` + `OwnerType`) so any entity type (Customer, Prospect, CreditApplication) can own documents without schema changes. Clients are responsible for uploading files to external storage before registering the URL.
