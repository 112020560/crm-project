...
- **THEN** all three columns are updated in the database, and `CustomerUpdated` is published with `Changes` containing the three updated keys
...
- **AND** `CustomerUpdated` is published with `Changes` containing only `CreditScore` and `MonthlyIncome`
...
- **THEN** the system returns HTTP 404 and no event is published
